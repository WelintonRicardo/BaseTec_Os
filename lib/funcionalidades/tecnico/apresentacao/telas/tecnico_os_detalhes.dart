...
class TecnicoOSDetalhes extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final segurado = os['nome_segurado'] ?? os['cliente'] ?? 'Não informado';

    final seguradora = os['seguradora'] ?? 'Não informado';

    final tipoServico = os['tipo_servico'] ?? 'Não informado';

    final status = os['status']?.toString() ?? 'PENDENTE';

    final telefone = os['telefone'] ?? '';

    final cidade = os['cidade'] ?? 'Não informado';

    final cep = os['cep'] ?? 'Não informado';

    final rua = os['rua'] ?? 'Não informado';

    final numero = os['numero'] ?? 'S/N';

    final complemento = os['complemento'] ?? '';

    final descricao =
        os['descricao_servico'] ?? os['descricao'] ?? 'Sem descrição';
    ...
    final podeIniciar = RegrasOS.podeIniciarExecucao(status);

    final isFinalizada = RegrasOS.isStatusFinal(status);
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // =================================================
            Card(
              color: isFinalizada ? Colors.white10 : AppCores.cardEscuro,
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const Divider(color: AppCores.bordaEscura, height: 24),
                    ...
                    if (complemento.toString().isNotEmpty)
                      Text(
              ...
              children: [
                Expanded(
                  child: ElevatedButton.icon(
                    ...
                    onPressed: () => _ligarCliente(telefone),

                    icon: const Icon(Icons.phone, color: AppCores.textoBranco),
                ...
                Expanded(
                  child: ElevatedButton.icon(
                    ...
                    icon: const Icon(Icons.chat, color: AppCores.textoBranco),
              ...
              onPressed: podeIniciar
                  ? () async {
                      ...
                      try {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text('Execução iniciada com sucesso!'),
                          ),
                        );
